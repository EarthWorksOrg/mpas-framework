...
! !PUBLIC MEMBER FUNCTIONS:
      public ESMFloc_TimeGet
      public ESMFloc_TimeSet
      public ESMFloc_TimePrint

! Required inherited and overridden ESMFloc_Base class methods

      public ESMFloc_TimeCopy
      public ESMFloc_SetYearWidth

! !PRIVATE MEMBER FUNCTIONS:

      private ESMFloc_TimeGetDayOfYear
      private ESMFloc_TimeGetDayOfYearInteger

! Inherited and overloaded from ESMFloc_BaseTime

      public operator(+)
      public ESMFloc_TimeInc

      public operator(-)
      private ESMFloc_TimeDec
      private ESMFloc_TimeDiff

      public operator(.EQ.)
      public ESMFloc_TimeEQ

      public operator(.NE.)
      public ESMFloc_TimeNE

      public operator(.LT.)
      public ESMFloc_TimeLT

      public operator(.GT.)
      public ESMFloc_TimeGT

      public operator(.LE.)
      public ESMFloc_TimeLE

      public operator(.GE.)
      public ESMFloc_TimeGE
...
!BOP
! !INTERFACE:
      interface ESMFloc_TimeGetDayOfYear

! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeGetDayOfYearInteger
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeInc, ESMFloc_TimeInc2
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeCopy
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeDec, ESMFloc_TimeDec2
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeDiff
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeEQ
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeNE
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeLT
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeGT
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeLE
...
! !PRIVATE MEMBER FUNCTIONS:
      module procedure ESMFloc_TimeGE
...
!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeGet - Get value in user-specified units

! !INTERFACE:
!      subroutine ESMFloc_TimeGet(time, YY, YRl, MM, DD, D, Dl, H, M, S, Sl, MS, &
!                              US, NS, d_, h_, m_, s_, ms_, us_, ns_, Sn, Sd, &
!                              dayOfYear, dayOfYear_r8, dayOfYear_intvl,      &
!                              timeString, rc)

recursive subroutine ESMFloc_TimeGet(time, YY, MM, DD, D, Dl, H, M, S, MS, &
                              Sn, Sd, &
                              dayOfYear, dayOfYear_r8, dayOfYear_intvl,      &
                              timeString, rc)

! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time
      integer, intent(out), optional :: YY
!      integer(ESMFloc_KIND_I8), intent(out), optional :: YRl
      integer, intent(out), optional :: MM
      integer, intent(out), optional :: DD
      integer, intent(out), optional :: D
      integer(ESMFloc_KIND_I8), intent(out), optional :: Dl
      integer, intent(out), optional :: H
      integer, intent(out), optional :: M
      integer, intent(out), optional :: S
!      integer(ESMFloc_KIND_I8), intent(out), optional :: Sl
...
!      double precision, intent(out), optional :: ns_
      ...
      real(ESMFloc_KIND_R8), intent(out), optional :: dayOfYear_r8
      character (len=*), intent(out), optional :: timeString
      type(ESMFloc_TimeInterval), intent(out), optional :: dayOfYear_intvl
...
!EOP
      type(ESMFloc_TimeInterval) :: day_step
      integer :: ierr
      TYPE(ESMFloc_Time) :: begofyear
      TYPE(ESMFloc_TimeInterval) :: difftobegofyear
      INTEGER :: year, month, dayofmonth, hour, minute, second
      INTEGER :: i
      INTEGER(ESMFloc_KIND_I8) :: cnt
...
!
!$$$ push HMS down into ESMFloc_BaseTime
      IF ( PRESENT( H ) ) THEN
      ...
      IF ( PRESENT( dayOfYear ) ) THEN
        CALL ESMFloc_TimeGetDayOfYear( time, dayOfYear, rc=ierr )
      ENDIF
...
!$$$ push HMS down into ESMFloc_BaseTime
      ...
      IF ( PRESENT( dayOfYear_intvl ) ) THEN
        year = time%YR
        CALL ESMFloc_TimeSet( begofyear, yy=year, mm=1, dd=1, s=0, &
                           calendar=time%calendar, rc=ierr )
        IF ( ierr == ESMFloc_FAILURE)THEN
           rc = ierr
           RETURN
        END IF
      ...
      IF ( PRESENT( dayOfYear_r8) ) THEN
        ...
        IF ( ierr == ESMFloc_FAILURE)THEN
           ...
           RETURN
        END IF
        CALL ESMFloc_TimeIntervalSet( day_step, d=1, s=0, rc=ierr )
        IF ( ierr == ESMFloc_FAILURE)THEN
           rc = ierr
           RETURN
        END IF
        difftobegofyear = time - begofyear + day_step
        CALL ESMFloc_TimeIntervalGet( difftobegofyear, d_r8=dayOfYear_r8, rc=ierr )
        IF ( ierr == ESMFloc_FAILURE)THEN
           rc = ierr
           RETURN
        END IF
      ...
      end subroutine ESMFloc_TimeGet

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeSet - Initialize via user-specified unit set

! !INTERFACE:
!      subroutine ESMFloc_TimeSet(time, YY, YRl, MM, DD, D, Dl, H, M, S, Sl, &
!                              MS, US, NS, d_, h_, m_, s_, ms_, us_, ns_, &
!                              Sn, Sd, calendar, calkindflag, rc)

      subroutine ESMFloc_TimeSet(time, YY, MM, DD, D, Dl, H, M, S, &
                              MS, &
                              Sn, Sd, calendar, calkindflag, rc)

! !ARGUMENTS:
      type(ESMFloc_Time), intent(inout) :: time
      integer, intent(in), optional :: YY
!      integer(ESMFloc_KIND_I8), intent(in), optional :: YRl
...
!      integer(ESMFloc_KIND_I8), intent(in), optional :: Sl
...
!      double precision, intent(in), optional :: ns_
      ...
      type(ESMFloc_Calendar), intent(in), target, optional :: calendar
      type(ESMFloc_CalKind_Flag), intent(in),  optional :: calkindflag
      integer, intent(out), optional :: rc
...
!     TMGn.n.n
!EOP
!  PRINT *,'DEBUG:  BEGIN ESMFloc_TimeSet()'
!$$$ push this down into ESMFloc_BaseTime constructor
...
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  using passed-in calendar'
        IF ( .not. ESMFloc_CalendarInitialized( calendar ) )THEN
           call wrf_error_fatal( "Error:: ESMFloc_CalendarCreate not "// &
                                 "called on input Calendar")
        END IF
!        call flush(6)
!        write(6,*) 'tcx1 ESMFloc_TimeSet point to calendar'
!        call flush(6)
        time%Calendar => calendar
      ELSE
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  using default calendar'
! for the sake of WRF, check ESMFloc_IsInitialized, revised by Juanxiong He
        IF ( .not. ESMFloc_IsInitialized() )THEN
           call wrf_error_fatal( "Error:: ESMFloc_Initialize not called")
        END IF
!        IF ( .not. ESMFloc_CalendarInitialized( defaultCal ) )THEN
!           call wrf_error_fatal( "Error:: ESMFloc_Initialize not called")
!        END IF
        if (present(calkindflag)) then
!           write(6,*) 'tcx2 ESMFloc_TimeSet point to calendarkindflag',calkindflag%caltype
!           call flush(6)
           if (calkindflag%caltype == ESMFloc_CALKIND_GREGORIAN%caltype) then
              time%Calendar => gregorianCal
           elseif (calkindflag%caltype == ESMFloc_CALKIND_NOLEAP%caltype) then
              time%Calendar => noleapCal
           else
              call wrf_error_fatal( "Error:: ESMFloc_TimeSet invalid calkindflag")
           endif
        else
!           write(6,*) 'tcx3 ESMFloc_TimeSet point to defaultcal'
!           call flush(6)
           time%Calendar => defaultCal
        endif
      END IF
!      write(6,*) 'tcxn ESMFloc_TimeSet ',ESMFloc_CALKIND_NOLEAP%caltype
!      call flush(6)
!      write(6,*) 'tcxg ESMFloc_TimeSet ',ESMFloc_CALKIND_GREGORIAN%caltype
!      call flush(6)
!      write(6,*) 'tcxt ESMFloc_TimeSet ',time%calendar%type%caltype
!      call flush(6)

      dset = .false.
      if (present(D)) then
         if (present(Dl)) CALL wrf_error_fatal( 'ESMFloc_TimeSet:  D and Dl not both valid')
         time%basetime%s = SECONDS_PER_DAY * INT(D-1,ESMFloc_KIND_I8)
         dset=.true.
      elseif (present(Dl)) then
         time%basetime%s = SECONDS_PER_DAY * Dl-1_ESMFloc_KIND_I8
         dset=.true.
      endif

      IF ( PRESENT( YY ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  YY = ',YY
        if (dset) CALL wrf_error_fatal( 'ESMFloc_TimeSet:  D or DL and YY,MM,DD not both valid')
        time%YR = YY
      ENDIF
      IF ( PRESENT( MM ) ) THEN
        if (dset) CALL wrf_error_fatal( 'ESMFloc_TimeSet:  D or DL and YY,MM,DD not both valid')
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  MM = ',MM
        ...
        IF ( ierr == ESMFloc_FAILURE ) THEN
          IF ( PRESENT( rc ) ) THEN
            ...
            RETURN
          ELSE
            CALL wrf_error_fatal( 'ESMFloc_TimeSet:  MM out of range' )
          ENDIF
        ENDIF
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  back from timeaddmonths'
      ENDIF
      IF ( PRESENT( DD ) ) THEN
        if (dset) CALL wrf_error_fatal( 'ESMFloc_TimeSet:  D or DL and YY,MM,DD not both valid')
!$$$ no check for DD in range of days of month MM yet
!$$$ Must separate D and DD for correct interface!
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  DD = ',DD
        time%basetime%S = time%basetime%S + &
          ( SECONDS_PER_DAY * INT( (DD-1), ESMFloc_KIND_I8 ) )
      ENDIF
!$$$ push H,M,S,Sn,Sd,MS down into ESMFloc_BaseTime constructor
      IF ( PRESENT( H ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  H = ',H
        time%basetime%S = time%basetime%S + &
          ( SECONDS_PER_HOUR * INT( H, ESMFloc_KIND_I8 ) )
      ENDIF
      IF ( PRESENT( M ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  M = ',M
        time%basetime%S = time%basetime%S + &
          ( SECONDS_PER_MINUTE * INT( M, ESMFloc_KIND_I8 ) )
      ENDIF
      IF ( PRESENT( S ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  S = ',S
        time%basetime%S = time%basetime%S + &
          INT( S, ESMFloc_KIND_I8 )
      ENDIF
      IF ( PRESENT( Sn ) .AND. ( .NOT. PRESENT( Sd ) ) ) THEN
        CALL wrf_error_fatal( &
          "ESMFloc_TimeSet:  Must specify Sd if Sn is specified")
      ENDIF
      IF ( PRESENT( Sd ) .AND. PRESENT( MS ) ) THEN
        CALL wrf_error_fatal( &
          "ESMFloc_TimeSet:  Must not specify both Sd and MS")
      ENDIF
      time%basetime%Sn = 0
      time%basetime%Sd = 0
      IF ( PRESENT( MS ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  MS = ',MS
        time%basetime%Sn = MS
        time%basetime%Sd = 1000_ESMFloc_KIND_I8
      ELSE IF ( PRESENT( Sd ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  Sd = ',Sd
        time%basetime%Sd = Sd
        IF ( PRESENT( Sn ) ) THEN
!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  Sn = ',Sn
          time%basetime%Sn = Sn
        ENDIF
      ENDIF

!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  calling normalize_time()'
!$$$DEBUG
!IF ( time%basetime%Sd > 0 ) THEN
!  PRINT *,'DEBUG ESMFloc_TimeSet() before normalize:  S,Sn,Sd = ', &
...
!$$$DEBUG
!IF ( time%basetime%Sd > 0 ) THEN
!  PRINT *,'DEBUG ESMFloc_TimeSet() after normalize:  S,Sn,Sd = ', &
!    time%basetime%S, time%basetime%Sn, time%basetime%Sd
!ENDIF
!$$$END DEBUG

!  PRINT *,'DEBUG:  ESMFloc_TimeSet():  back from normalize_time()'
      IF ( PRESENT( rc ) ) THEN
        rc = ESMFloc_SUCCESS
      ENDIF

      end subroutine ESMFloc_TimeSet
...
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  YR,S,Sn,Sd = ',time%YR,time%basetime%S,time%basetime%Sn,time%basetime%Sd
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  year = ',year
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  month, dayofmonth = ',month,dayofmonth
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  hour = ',hour
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  minute = ',minute
!PRINT *,'DEBUG:  ESMFloc_TimePrint():  second = ',second
...
!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeGetDayOfYearInteger - Get time instant's day of the year as an integer value
!
! !INTERFACE:
      subroutine ESMFloc_TimeGetDayOfYearInteger(time, DayOfYear, rc)
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time
...
!$$$ add tests
      ...
      end subroutine ESMFloc_TimeGetDayOfYearInteger

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeInc - Increment time instant with a time interval
!
! !INTERFACE:
   function ESMFloc_TimeInc(time, timeinterval)
!
! !RETURN VALUE:
      type(ESMFloc_Time) :: ESMFloc_TimeInc
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time
...
!EOP
      ...
      ! copy ESMFloc_Time specific properties (e.g. calendar, timezone)

      ESMFloc_TimeInc = time
!      write(6,*) 'tcx timeinc1 ',ESMFloc_TimeInc%yr,ESMFloc_TimeInc%basetime%s
      CALL normalize_time( ESMFloc_TimeInc )
...
!      write(6,*) 'tcx timeint ',timeinterval%yr,timeinterval%mm,timeinterval%basetime%s
      ...
      if (abs(nmon) > 0 .or. abs(nyr) > 0) then
         call ESMFloc_TimeGet(ESMFloc_TimeInc,yy=year,mm=month,dd=day,s=sec)
...
!         write(6,*) 'tcx timeinc mon2 ',year,month,day,sec
         day = min(day,ndaysinmonth(year,month,ESMFloc_TimeInc%calendar%type))
         call ESMFloc_TimeSet(ESMFloc_TimeInc,yy=year,mm=month,dd=day,s=sec,calkindflag=time%calendar%type)
         call ESMFloc_TimeGet(ESMFloc_TimeInc,yy=year,mm=month,dd=day,s=sec)
...
!      write(6,*) 'tcx timeinc sec ',ESMFloc_TimeInc%basetime%s,timeinterval%basetime%s
      ESMFloc_TimeInc%basetime = ESMFloc_TimeInc%basetime + timeinterval%basetime

      ! and normalize

!      write(6,*) 'tcx timeinc2p ',ESMFloc_TimeInc%yr,ESMFloc_TimeInc%basetime%s

      CALL normalize_time( ESMFloc_TimeInc )

!      write(6,*) 'tcx timeinc2 ',ESMFloc_TimeInc%yr,ESMFloc_TimeInc%basetime%s

   end function ESMFloc_TimeInc

! this is added for certain compilers that don't deal with commutativity

   function ESMFloc_TimeInc2(timeinterval, time)
      type(ESMFloc_Time) :: ESMFloc_TimeInc2
      type(ESMFloc_Time), intent(in) :: time
      type(ESMFloc_TimeInterval), intent(in) :: timeinterval
      ESMFloc_TimeInc2 = ESMFloc_TimeInc( time, timeinterval )
   end function ESMFloc_TimeInc2

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeDec - Decrement time instant with a time interval
!
! !INTERFACE:
   function ESMFloc_TimeDec(time, timeinterval)
!
! !RETURN VALUE:
      type(ESMFloc_Time) :: ESMFloc_TimeDec
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time
      type(ESMFloc_TimeInterval), intent(in) :: timeinterval
! !LOCAL:
      TYPE (ESMFloc_TimeInterval)  :: neginterval
...
!$$$push this down into a unary negation operator on TimeInterval
      ...
      ESMFloc_TimeDec = time + neginterval

   end function ESMFloc_TimeDec

!
! this is added for certain compilers that don't deal with commutativity
!
   function ESMFloc_TimeDec2(timeinterval, time)
      type(ESMFloc_Time) :: ESMFloc_TimeDec2
      type(ESMFloc_Time), intent(in) :: time
      type(ESMFloc_TimeInterval), intent(in) :: timeinterval
      ESMFloc_TimeDec2 = ESMFloc_TimeDec( time, timeinterval )
   end function ESMFloc_TimeDec2
!
!------------------------------------------------------------------------------
!BOP
! !IROUTINE:  ESMFloc_TimeDiff - Return the difference between two time instants
!
! !INTERFACE:
   function ESMFloc_TimeDiff(time1, time2)
!
! !RETURN VALUE:
      type(ESMFloc_TimeInterval) :: ESMFloc_TimeDiff
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
! !LOCAL:
      TYPE(ESMFloc_BaseTime) :: cmptime, zerotime
      integer :: yr
...
!EOP
      ...
      CALL ESMFloc_TimeIntervalSet( ESMFloc_TimeDiff, rc=rc )

      ESMFloc_TimeDiff%StartTime = time2
      ESMFloc_TimeDiff%StartTime_set = .true.
...
!      write(6,*) 'tcx timediff2 ',time1%yr,time1%basetime%s,time1%calendar%type%caltype
      ...
      call ESMFloc_TimeGet(time2,yy=y2,mm=m2,dd=d2,s=s2)
      call ESMFloc_TimeGet(time1,yy=y1,mm=m1,dd=d1,s=s1)

      ! Can either be yr/month based diff if diff is only in year and month
      ! or absolute seconds if diff in day/seconds as well
      !
      ! Update: Actually, the timeintcmp() routine in ESMFloc_TimeIntervalMod.F90 is not capable
...
!         ESMFloc_TimeDiff%YR = y1 - y2
!         ESMFloc_TimeDiff%MM = m1 - m2
!         cmptime%S  = 0
!         cmptime%Sn = 0
!         cmptime%Sd = 0
!         ESMFloc_TimeDiff%basetime = cmptime
!      else
!         write(6,*) 'tcx timedifft sec'
         ESMFloc_TimeDiff%YR = 0
         ESMFloc_TimeDiff%MM = 0
         ESMFloc_TimeDiff%basetime = time1%basetime - time2%basetime
         IF ( time1%YR > time2%YR ) THEN
            DO yr = time2%YR, ( time1%YR - 1 )
!              write(6,*) 'tcx timediff3 ',yr,nsecondsinyear(yr,time2%calendar%type)
              ESMFloc_TimeDiff%basetime%S = ESMFloc_TimeDiff%basetime%S + nsecondsinyear(yr,time2%calendar%type)
            ENDDO
...
!      write(6,*) 'tcx timediff5 ',ESMFloc_TimeDiff%YR, ESMFloc_TimeDiff%MM, ESMFloc_TimeDiff%basetime%s

      CALL normalize_timeint( ESMFloc_TimeDiff )

!      write(6,*) 'tcx timediff6 ',ESMFloc_TimeDiff%YR, ESMFloc_TimeDiff%MM, ESMFloc_TimeDiff%basetime%s

   end function ESMFloc_TimeDiff

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeEQ - Compare two times for equality
!
! !INTERFACE:
      function ESMFloc_TimeEQ(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeEQ
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      ESMFloc_TimeEQ = (res .EQ. 0)

      end function ESMFloc_TimeEQ

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeNE - Compare two times for non-equality
!
! !INTERFACE:
      function ESMFloc_TimeNE(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeNE
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      call timecmp(time1,time2,res)
      ESMFloc_TimeNE = (res .NE. 0)

      end function ESMFloc_TimeNE

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeLT - Time instant 1 less than time instant 2 ?
!
! !INTERFACE:
      function ESMFloc_TimeLT(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeLT
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      call timecmp(time1,time2,res)
      ESMFloc_TimeLT = (res .LT. 0)

      end function ESMFloc_TimeLT

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeGT - Time instant 1 greater than time instant 2 ?
!
! !INTERFACE:
      function ESMFloc_TimeGT(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeGT
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      call timecmp(time1,time2,res)
      ESMFloc_TimeGT = (res .GT. 0)

      end function ESMFloc_TimeGT

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeLE - Time instant 1 less than or equal to time instant 2 ?
!
! !INTERFACE:
      function ESMFloc_TimeLE(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeLE
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      call timecmp(time1,time2,res)
      ESMFloc_TimeLE = (res .LE. 0)

      end function ESMFloc_TimeLE

!------------------------------------------------------------------------------
!BOP
! !IROUTINE: ESMFloc_TimeGE - Time instant 1 greater than or equal to time instant 2 ?
!
! !INTERFACE:
      function ESMFloc_TimeGE(time1, time2)
!
! !RETURN VALUE:
      logical :: ESMFloc_TimeGE
!
! !ARGUMENTS:
      type(ESMFloc_Time), intent(in) :: time1
      type(ESMFloc_Time), intent(in) :: time2
...
!EOP
      ...
      call timecmp(time1,time2,res)
      ESMFloc_TimeGE = (res .GE. 0)

      end function ESMFloc_TimeGE

!------------------------------------------------------------------------------
!BOP
! !IROUTINE:  ESMFloc_TimeCopy - Copy a time-instance

! !INTERFACE:
      subroutine ESMFloc_TimeCopy(timeout, timein)

! !ARGUMENTS:
      type(ESMFloc_Time), intent(out) :: timeout
      type(ESMFloc_Time), intent(in) :: timein
...
!      write(6,*) 'tcxa ESMFloc_TimeCopy'
!      call flush(6)
!      write(6,*) 'tcxb ESMFloc_TimeCopy',timein%calendar%type%caltype
!      call flush(6)
      timeout%Calendar = ESMFloc_CalendarCreate(calkindflag=timein%calendar%type)

      end subroutine ESMFloc_TimeCopy


!------------------------------------------------------------------------------
!BOP
! !IROUTINE:  ESMFloc_TimePrint - Print out a time instant's properties


! !INTERFACE:
      subroutine ESMFloc_TimePrint(time, options, rc)
...
!EOP
      ...
      IF ( PRESENT( options ) ) THEN
        CALL ESMFloc_TimeGet( time, timeString=timestr, rc=rc )
      ...
      end subroutine ESMFloc_TimePrint

!==============================================================================

SUBROUTINE print_a_time( time )
   IMPLICIT NONE
   type(ESMFloc_Time) time
   character*128 :: s
   integer rc
   CALL ESMFloc_TimeGet( time, timeString=s, rc=rc )
...
SUBROUTINE normalize_time( time )
  ...
  IMPLICIT NONE
  TYPE(ESMFloc_Time), INTENT(INOUT) :: time
!  INTEGER(ESMFloc_KIND_I8) :: nsecondsinyear
  ! locals
  TYPE(ESMFloc_BaseTime) :: cmptime, zerotime
  INTEGER :: rc
  LOGICAL :: done
...
! 1 <= MM <= 12
! Time is NOT normalized.
SUBROUTINE ESMFloc_setYearWidth( yearWidthIn )

    integer, intent(in) :: yearWidthIn

    yearWidth = yearWidthIn

END SUBROUTINE ESMFloc_setYearWidth

!==============================================================================
!==============================================================================
end module ESMFloc_TimeMod
