...
!------------------------------------------------------------------------------
      ...
      TYPE(ESMFloc_BaseTime), TARGET :: monthbdys(0:MONTHS_PER_YEAR)
      TYPE(ESMFloc_BaseTime), TARGET :: monthbdysleap(0:MONTHS_PER_YEAR)
      TYPE(ESMFloc_BaseTime), TARGET :: monthedys(0:MONTHS_PER_YEAR)
      TYPE(ESMFloc_BaseTime), TARGET :: monthedysleap(0:MONTHS_PER_YEAR)


!------------------------------------------------------------------------------
!     ! ESMFloc_CalKind_Flag
!
!     ! F90 "enum" type to match C++ ESMC_CalKind_Flag enum

      type ESMFloc_CalKind_Flag
        integer :: caltype
      end type

      type(ESMFloc_CalKind_Flag), parameter :: &
                               ESMFloc_CALKIND_GREGORIAN =  ESMFloc_CalKind_Flag(1), &
                               ESMFloc_CALKIND_NOLEAP =     ESMFloc_CalKind_Flag(2)

!      type(ESMFloc_CalKind_Flag), parameter :: &
!                               ESMFloc_CALKIND_GREGORIAN =  ESMFloc_CalKind_Flag(1), &
!                               ESMFloc_CALKIND_JULIAN =     ESMFloc_CalKind_Flag(2), &
!                           ! like Gregorian, except Feb always has 28 days
!                               ESMFloc_CALKIND_NOLEAP =     ESMFloc_CalKind_Flag(3), &
!                           ! 12 months, 30 days each
!                               ESMFloc_CALKIND_360DAY =     ESMFloc_CalKind_Flag(4), &
!                           ! user defined
!                               ESMFloc_CALKIND_GENERIC =    ESMFloc_CalKind_Flag(5), &
!                           ! track base time seconds only
!                               ESMFloc_CALKIND_NOCALENDAR = ESMFloc_CalKind_Flag(6)
...
!
!
      type ESMFloc_Calendar
        type(ESMFloc_CalKind_Flag) :: Type
        logical :: Set = .false.
        integer, dimension(MONTHS_PER_YEAR) :: DaysPerMonth = 0
        integer :: SecondsPerDay = 0
        integer :: SecondsPerYear = 0
        type(ESMFloc_DaysPerYear) :: DaysPerYear
      end type
!------------------------------------------------------------------------------
! !PUBLIC DATA: added by Juanxiong He, in order to breakthe cycle call between
! ESMFloc_Stubs and ESMFloc_Time
   TYPE(ESMFloc_Calendar), public, save, pointer :: defaultCal   ! Default Calendar
   TYPE(ESMFloc_Calendar), public, save, pointer :: gregorianCal ! gregorian Calendar
   TYPE(ESMFloc_Calendar), public, save, pointer :: noleapCal    ! noleap Calendar
...
!      public mdayleapcum
      ...
      public isleap
      public ESMFloc_CalKind_Flag
      public ESMFloc_CALKIND_GREGORIAN, ESMFloc_CALKIND_NOLEAP
!             ESMFloc_CALKIND_360DAY, ESMFloc_CALKIND_NOCALENDAR
!      public ESMFloc_CAL_JULIAN
!      public ESMFloc_CAL_GENERIC
      public ESMFloc_Calendar
      public ESMFloc_DaysPerYear

!------------------------------------------------------------------------------
!
! !PUBLIC MEMBER FUNCTIONS:
      public ESMFloc_CalendarCreate

! Required inherited and overridden ESMFloc_Base class methods

      public ESMFloc_CalendarInitialized ! Only in this implementation, intended
...
!==============================================================================
!BOP
! !IROUTINE: ESMFloc_CalendarCreate - Create a new ESMF Calendar of built-in type

! !INTERFACE:
      ! Private name; call using ESMFloc_CalendarCreate()
      function ESMFloc_CalendarCreate(name, calkindflag, rc)

! !RETURN VALUE:
      type(ESMFloc_Calendar) :: ESMFloc_CalendarCreate

! !ARGUMENTS:
      character (len=*),       intent(in),  optional :: name
      type(ESMFloc_CalKind_Flag), intent(in)            :: calkindflag
...
! !REQUIREMENTS:
!     TMGn.n.n
      type(ESMFloc_DaysPerYear) :: dayspy

      if ( present(rc) ) rc = ESMFloc_FAILURE
! Calendar is hard-coded.  Use ESMF library if more flexibility is needed.
!      write(6,*) 'tcx ESMFloc_CalendarCreate ',calkindflag%caltype, ESMFloc_CALKIND_NOLEAP%caltype, ESMFloc_CALKIND_GREGORIAN%caltype
      if ( calkindflag%caltype  == ESMFloc_CALKIND_NOLEAP%caltype ) then
!         write(6,*) 'tcx ESMFloc_CalendarCreate: initialize noleap calendar '
         ESMFloc_CalendarCreate%Type = ESMFloc_CALKIND_NOLEAP
      elseif ( calkindflag%caltype  == ESMFloc_CALKIND_GREGORIAN%caltype ) then
!         write(6,*) 'tcx ESMFloc_CalendarCreate: initialize gregorian calendar '
         ESMFloc_CalendarCreate%Type = ESMFloc_CALKIND_GREGORIAN
      else
!         write(6,*) 'tcx ESMFloc_CalendarCreate: ERROR initialize invalid calendar'
         call wrf_error_fatal( "Error:: ESMFloc_CalendarCreate invalid calendar")
      endif

!$$$ This is a bug on some systems -- need initial value set by compiler at
!$$$ startup.
      ESMFloc_CalendarCreate%Set = .true.
      ESMFloc_CalendarCreate%SecondsPerDay = SECONDS_PER_DAY
! DaysPerYear and SecondsPerYear are incorrect for Gregorian calendars...
      dayspy%D = size(daym)
      dayspy%Dn = 0
      dayspy%Dd = 1
      ESMFloc_CalendarCreate%DaysPerYear = dayspy
      ESMFloc_CalendarCreate%SecondsPerYear = ESMFloc_CalendarCreate%SecondsPerDay &
                                       * dayspy%D
      ESMFloc_CalendarCreate%DaysPerMonth(:) = mday(:)

      if ( present(rc) ) rc = ESMFloc_SUCCESS

      end function ESMFloc_CalendarCreate


!==============================================================================
!BOP
! !IROUTINE: ESMFloc_CalendarInitialized - check if calendar was created

! !INTERFACE:
      function ESMFloc_CalendarInitialized(calendar)

! !RETURN VALUE:
      logical ESMFloc_CalendarInitialized

! !ARGUMENTS:
      type(ESMFloc_Calendar), intent(in)            :: calendar

! !DESCRIPTION:
!EOP
! !REQUIREMENTS:
!     TMGn.n.n
        ESMFloc_CalendarInitialized = calendar%set
        if ( calendar%SecondsPerDay == 0 ) &
              ESMFloc_CalendarInitialized = .false.

     end function ESMFloc_CalendarInitialized
...
!$$$ push this down into ESMFloc_BaseTime constructor
    monthbdys(i)%S  = SECONDS_PER_DAY * INT( mdaycum(i), ESMFloc_KIND_I8 )
    monthbdys(i)%Sn = 0
    monthbdys(i)%Sd = 0
  ENDDO
...
!$$$ push this down into ESMFloc_BaseTime constructor
    monthbdysleap(i)%S  = SECONDS_PER_DAY * INT( mdayleapcum(i), ESMFloc_KIND_I8 )
...
integer(ESMFloc_kind_i8) FUNCTION nsecondsinyear ( year, calkindflag )
  ! Compute the number of seconds in the given year
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: year
  type(ESMFloc_CalKind_Flag),intent(in) :: calkindflag

  nsecondsinyear = SECONDS_PER_DAY * INT( ndaysinyear(year, calkindflag) , ESMFloc_KIND_I8 )
...
integer(ESMFloc_kind_i8) function nsecondsinmonth( year,month,calkindflag)
  ! Compute number of days in month for year, month, cal
  IMPLICIT NONE
  INTEGER, INTENT(in) :: year,month
  type(ESMFloc_CalKind_Flag),intent(in) :: calkindflag
...
integer function nmonthinyearsec(year,basetime,calkindflag)
  ...
  type(ESMFloc_CalKind_Flag),intent(in) :: calkindflag
  ! locals
  TYPE(ESMFloc_BaseTime), pointer :: MMbdys(:)
  integer :: mm,i
...
integer function ndayinyearsec(year, basetime, calkindflag)
  ...
  ! locals
  TYPE(ESMFloc_BaseTime), pointer :: MMbdys(:)
  TYPE(ESMFloc_BaseTime) :: tmpbasetime
  integer :: mm
...
!==============================================================================
integer(ESMFloc_kind_i8) function nsecondsinyearmonth(year, month, calkindflag)
...
logical FUNCTION isleap ( year, calkindflag )
  ...
  IMPLICIT NONE
  INTEGER,intent(in)  :: year
  type(ESMFloc_CalKind_Flag) :: calkindflag
  ! local
  INTEGER :: lyear
...
!==============================================================================
end module ESMFloc_CalendarMod
