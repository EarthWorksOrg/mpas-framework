...
int parse_packages_from_registry(ezxml_t registry)/*{{{*/
{
	...
	fortprintf(fd, "      use MPASAOS_derived_types\n");
	fortprintf(fd, "      use MPASAOS_pool_routines\n");
	fortprintf(fd, "      use MPASAOS_io_units\n");
...
int parse_namelist_records_from_registry(ezxml_t registry)/*{{{*/
{
	...
	fortprintf(fd2, "      use MPASAOS_derived_types\n");
	fortprintf(fd2, "      use MPASAOS_pool_routines\n");
	fortprintf(fd2, "      use MPASAOS_io_units\n");
	fortprintf(fd2, "      use MPASAOS_abort, only : mpas_dmpar_global_abort\n");
	fortprintf(fd2, "      use MPASAOS_log, only : mpas_log_write\n");
	...
	for (nmlrecs_xml = ezxml_child(registry, "nml_record"); nmlrecs_xml; nmlrecs_xml = nmlrecs_xml->next){
		...
		fortprintf(fd, "      use MPASAOS_log, only : mpas_log_write, mpas_log_escape_dollars\n");
		fortprintf(fd, "      implicit none\n");
...
int parse_dimensions_from_registry(ezxml_t registry)/*{{{*/
{
	...
	fortprintf(fd, "      use MPASAOS_pool_routines\n");
	fortprintf(fd, "      use MPASAOS_io_units\n");
	fortprintf(fd, "      use MPASAOS_log, only : mpas_log_write\n");
	fortprintf(fd, "\n");
	fortprintf(fd, "      implicit none\n");
	fortprintf(fd, "\n");
	...
	fortprintf(fd, "\n");
	fortprintf(fd, "      use MPASAOS_derived_types\n");
	fortprintf(fd, "      use MPASAOS_decomp\n");
	fortprintf(fd, "      use MPASAOS_pool_routines\n");
	fortprintf(fd, "      use MPASAOS_io_units\n");
	fortprintf(fd, "      use MPASAOS_abort, only : mpas_dmpar_global_abort\n");
	fortprintf(fd, "      use MPASAOS_log, only : mpas_log_write\n");
	fortprintf(fd, "\n");
	fortprintf(fd, "      implicit none\n");
	fortprintf(fd, "\n");
...
int parse_struct(FILE *fd, ezxml_t registry, ezxml_t superStruct, int subpool, const char *parentname, const char * corename)/*{{{*/
{
	...
	fortprintf(fd, "      use MPASAOS_pool_routines\n");
	fortprintf(fd, "      use MPASAOS_io_units\n");
	fortprintf(fd, "      use MPASAOS_io, only : MPAS_REAL_FILLVAL, MPAS_INT_FILLVAL, MPAS_CHAR_FILLVAL\n");
	fortprintf(fd, "      implicit none\n");
...
int generate_immutable_streams(ezxml_t registry){/*{{{*/
	...
	fortprintf(fd, "   use MPASAOS_derived_types, only : MPAS_streamManager_type, &\n");
	...
	fortprintf(fd, "   use MPASAOS_stream_manager, only : MPAS_stream_mgr_create_stream, MPAS_stream_mgr_set_property, &\n");
