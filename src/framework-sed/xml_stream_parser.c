 ...
 */
void MPASAOS_stream_mgr_create_stream_c(void *, const char *, int *, const char *, const char *, const char *, const char *, int *, int *, int *, int *, int *, int *);
void MPASAOS_stream_mgr_add_field_c(void *, const char *, const char *, const char *, int *);
void MPASAOS_stream_mgr_add_immutable_stream_fields_c(void *, const char *, const char *, const char *, int *);
void MPASAOS_stream_mgr_add_pool_c(void *, const char *, const char *, const char *, int *);
void MPASAOS_stream_mgr_add_alarm_c(void *, const char *, const char *, const char *, const char *, int *);
void MPASAOS_stream_mgr_add_pkg_c(void *, const char *, const char *, int *);
 ...
 */
void MPASAOS_log_write_c(const char *message_c, const char *messageType_c);
...
void fmt_err(const char *mesg)
{
	...
	MPASAOS_log_write_c(msgbuf, "MPAS_LOG_ERR");
}
...
void fmt_warn(const char *mesg)
{
	...
	snprintf(msgbuf, MSGSIZE, "In file %s, %s", global_file, mesg);
	MPASAOS_log_write_c(msgbuf, "MPAS_LOG_WARN");
}
...
void fmt_info(const char *mesg)
{
	...
	snprintf(msgbuf, MSGSIZE, "In file %s, %s", global_file, mesg);
	MPASAOS_log_write_c(msgbuf, "MPAS_LOG_OUT");
}
...
void xml_stream_parser(char *fname, void *manager, int *mpi_comm, int *status)
{
	...
	*status = 0;
	...
	for (stream_xml = ezxml_child(streams, "immutable_stream"); stream_xml; stream_xml = ezxml_next(stream_xml)) {
		...
		MPASAOS_stream_mgr_create_stream_c(manager, streamID, &itype, filename_template, filename_interval_string, ref_time_local, rec_intv_local,
		...
		if (itype == 3 || itype == 1) {
			MPASAOS_stream_mgr_add_alarm_c(manager, streamID, "input", "start", interval_in2, &err);
			if (err != 0) {
				*status = 1;
				return;
			}
		...
		if (itype == 3 || itype == 2) {
			MPASAOS_stream_mgr_add_alarm_c(manager, streamID, "output", "start", interval_out2, &err);
			if (err != 0) {
				*status = 1;
				return;
			}
	...
	for (stream_xml = ezxml_child(streams, "stream"); stream_xml; stream_xml = ezxml_next(stream_xml)) {
		...
		for (varfile_xml = ezxml_child(stream_xml, "file"); varfile_xml; varfile_xml = ezxml_next(varfile_xml)) {
			...
			if (fd != NULL) {
				while (fscanf(fd, "%s", fieldname) != EOF) {
					MPASAOS_stream_mgr_add_field_c(manager, streamID, (const char *)fieldname, packages_local, &err);
					if (err != 0) {
						*status = 1;
						return;
					}
		...
		for (var_xml = ezxml_child(stream_xml, "var"); var_xml; var_xml = ezxml_next(var_xml)) {
			...
			MPASAOS_stream_mgr_add_field_c(manager, streamID, fieldname_const, packages_local, &err);
			if (err != 0) {
				*status = 1;
				return;
			}
		...
		for (varstruct_xml = ezxml_child(stream_xml, "var_struct"); varstruct_xml; varstruct_xml = ezxml_next(varstruct_xml)) {
			...
			MPASAOS_stream_mgr_add_pool_c(manager, streamID, structname_const, packages_local, &err);
			if (err != 0){
				*status = 1;
				return;
			}
		...
		for (substream_xml = ezxml_child(stream_xml, "stream"); substream_xml; substream_xml = ezxml_next(substream_xml)) {
			 ...
			 */
			MPASAOS_stream_mgr_add_immutable_stream_fields_c(manager, streamID, streamname_const, packages_local, &err);
			if (err != 0) {
